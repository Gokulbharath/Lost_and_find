    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
        ...
        <div className="max-w-5xl mx-auto px-4 py-12 text-center">
          ...
          <Link to="/search" className="text-blue-600 hover:text-blue-700">Back to search</Link>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="relative h-96 lg:h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700">
              ...
              <div className="absolute top-4 right-4">
                <span className={`px-4 py-2 rounded-full text-sm font-semibold ${isLost ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                  {isLost ? 'Lost' : 'Found'}
                </span>
              </div>
            </div>
            <div className="p-8">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-gray-500 dark:text-gray-400">{dateLabel}</div>
                    <div className="text-gray-900 dark:text-white">{new Date(date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                  {String(item.contact_info).includes('@') ? (
                    <>
                      <Mail className="w-5 h-5" />
                      <a href={`mailto:${item.contact_info}`} className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">{item.contact_info}</a>
                    </>
                  ) : (
                    <>
                      <Phone className="w-5 h-5" />
                      <a href={`tel:${item.contact_info}`} className="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">{item.contact_info}</a>
                    </>
                  )}
                </div>
              </div>
